...
def bubble_sort(unordered_list):
    iteration_number = len(unordered_list)
    for i in range(iteration_number-1):
        for j in range(iteration_number-i-1):
...
class Deck:
    ...
    def sort(self):  #TODO Implement a method to sort cards by suit and value;
        bubble_sort(self.cards) # Sort the cards
        self._populate_index_map()
    ...
    def search(self):  #TODO Implement a public search method;
        ...
        print("Card entered is: ", card)
        # constant look up as we populate index map
        index = self._index_map[card]
        
        print("Found the card at index position: ", self._index_map[card])
